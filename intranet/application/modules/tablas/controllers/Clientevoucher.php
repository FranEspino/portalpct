<?php 
class Clientevoucher extends CI_Controller {
    public function __construct() {
        parent::__construct();
        $this->load->model("Cliente_voucher_model");
		    }
    

     public function gridClienteVoucher(){ 
        $data['grid'] =  $this->Cliente_voucher_model->gridClienteVoucher();
       // var_dump($data['grid'] );
		$data['titulotabla'] = 'Lista de Clientes Pendientes';

        $this->load->view("gridClienteVoucher",$data);
    }  

    public function mantClienteVoucherForm($id_voucher = ''){	
		$data['estado_id'] = $this->proveedor_model->rowProveedor($id_voucher);
				if($id_voucher == " "){
		$data['titulofrm'] = "Agregar Registros";
		}else{
			$data['titulofrm'] = "Confirmar Voucher";
			}
        $this->load->view("form_MantClienteVoucher_view",$data);
    }
 	/*
	public function mantClienteVoucher($idproveedor = ''){	
	...
	public function mantClienteVoucher(){		 
