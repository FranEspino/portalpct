...
class Corporacion extends CI_Controller {
	...
	public function mantCorporacion(){
     ...
     if ($post['flg_codprod']==1) {
		 		$post['pref_cod'] = $this->input->post('pref_cod');//prefijo de código opcinal
		 }
		...
		$namec = 'PermitirAgIGV';    	
    	$postc['value'] = $this->input->post('permitir_agigv');
    	...
    	$namec = 'DetraccionVenta';    	
    	$postc['value'] = $this->input->post('detraccion_venta');
    	$infoconfig = $this->corporacion_model->rowConfig($namec);
    	if($infoconfig){
    		$this->corporacion_model->updateConfig($namec,$postc);
    	}else{
    	...
    	$namec = 'CantDelTotalVenta';    	
    	$postc['value'] = $this->input->post('CantDelTotalVenta');
    	$infoconfig = $this->corporacion_model->rowConfig($namec);
    	if($infoconfig){
    		$this->corporacion_model->updateConfig($namec,$postc);
    	}else{
    	...
    	$namec = 'BuscarPrecioVenta';    	
    	$postc['value'] = $this->input->post('BuscarPrecioVenta');
    	$infoconfig = $this->corporacion_model->rowConfig($namec);
    	if($infoconfig){
    		$this->corporacion_model->updateConfig($namec,$postc);
    	}else{
    	...
    	$namec = 'TipoDescVenta';    	
    	$postc['value'] = $this->input->post('TipoDescVenta');
    	$infoconfig = $this->corporacion_model->rowConfig($namec);
    	if($infoconfig){
    		$this->corporacion_model->updateConfig($namec,$postc);
    	}else{
