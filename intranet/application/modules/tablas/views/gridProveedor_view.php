...
<div id="content">			
    <div class="row ">     
        <div class="col-md-8">
            <h1>
                <i class="fa fa-edit fa-fw "></i> Proveedores
            </h1>   
        </div>        
        <div class="col-md-4 ">   
                            ...
                            <table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
                            	<thead>			                
                                    <tr>
                                        <th>N.</th>
                                         <th>RUC</th>
                                        <th><i class="fa fa-fw fa-user text-muted"></i>Proveedor</th>
                                         <th>Tlf - Cel</th>
                                         <th>Contacto</th>
                                         <th>Giro</th>
                                         <th>Ciudad</th>
								...
								<?php  if ($grid) {
                                     ...
                                     <tr id="<?php echo $fila->proveedor_id; ?>">
                                        <td><?php  echo $i; ?></td>
                                        ...
                                        <td><?php  echo $fila->condicioncompra; ?></td>
                                        
                                        <td class="pull-right opciones hidden-print">
                                        <button  id="btn_editardato_<?php  echo $fila->proveedor_id ; ?>" name="editardato_<?php  echo $fila->proveedor_id ; ?>" value="<?php  echo $fila->proveedor_id ; ?>" class="editardato btn btn-primary btn-circle"  data-placement="left" rel="tooltip" data-original-title="Editar"><i class="glyphicon glyphicon-edit"></i></button>
                                        <a href="javascript:void(0);" onclick="elim_reg(<?php echo $fila->proveedor_id;?>);" class="btn btn-danger btn-circle" rel="tooltip" data-placement="bottom" title="Eliminar" ><i class="glyphicon glyphicon-trash"></i></a>
                                       <!-- <a href="javascript:void(0);" class="btn btn-danger btn-circle" data-trigger="focus" rel="popover" data-placement="bottom" data-original-title="<i class='fa fa-warning'></i> ¿Eliminar?" data-content='<button type="button" class="btn btn-default btn-sm" title="No eliminar"><span class="glyphicon glyphicon-menu-left"></span>Cancelar</button><legend class="no-padding no-margin" style="margin:2px !important"></legend><button onclick="elim_reg(<?php echo $fila->proveedor_id;?>);" type="button" class="btn btn-danger btn-sm btn_si_eliminar" title="Si eliminar" ><span class="glyphicon glyphicon-trash"></span> Eliminar</button>' data-html="true"><i class="glyphicon glyphicon-trash"></i></a>-->
                                        </td>
                                    </tr> 
                                   <?php
								   $i++; 
...
function elim_reg(id){	
 var texto  = $("#" + id).find("td")[2].innerHTML.trim();
 ...
 Swal.fire({
  ...
  if (result.value) {
	...
	if(idtabla.length<1){  
		...
		$.ajax({ 
			type: "post",
			url: "<?php  echo base_url() ?>tablas/proveedor/eliminarProveedor", cache: false, 
...
$(".editardato").click(function(event){
	...
	var url = "<?php  echo base_url() ?>tablas/proveedor/mantProveedorForm/"+idtablas;
	$( "#div_ModalMant" ).load(url);
});  

$("#btnnuevo").click(function(event){ 
	event.preventDefault();        
	var urlnuevo = "<?php  echo base_url() ?>tablas/proveedor/mantProveedorForm/";
