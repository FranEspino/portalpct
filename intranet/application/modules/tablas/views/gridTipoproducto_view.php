...
<div id="content">			
    <div class="row ">     
        <div class="col-xs-11">
            <h1>
                <i class="fa fa-edit fa-fw "></i> Categoria del Producto
            </h1>   
        </div>        
        <div class="col-xs-1 ">   
                        ...
                        <div class="table-responsive">
                            <table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
                            	<thead>			                
                                    <tr>
                                        <th>N.</th>
                                        <th><i class="fa fa-fw fa-database text-muted"></i>Categoria</th>
								...
								<?php  if ($grid) {
                                     ...
                                     <tr id="<?php echo $fila->tipoproducto_id; ?>">
                                        <td><?php  echo $i; ?></td>
                                        <td ><?php  echo $fila->tipoproducto  ; ?></td> 
                                        <td class="pull-right opciones hidden-print">
                                        <button  id="btn_editardato_<?php  echo $fila->tipoproducto_id ; ?>" name="editardato_<?php  echo $fila->tipoproducto_id ; ?>" value="<?php  echo $fila->tipoproducto_id ; ?>" class="editardato btn btn-primary btn-circle"  data-placement="left" rel="tooltip" data-original-title="Editar"><i class="glyphicon glyphicon-edit"></i></button>
                                        <a href="javascript:void(0);" onclick="elim_reg(<?php echo $fila->tipoproducto_id;?>);" class="btn btn-danger btn-circle" rel="tooltip" data-placement="bottom" title="Eliminar" ><i class="glyphicon glyphicon-trash"></i></a>
                                        </td>
                                    </tr> 
                                   <?php
								   $i++; 
 ...
 Swal.fire({
  ...
  if (result.value) {
	...
	if(idtabla.length<1){  
		...
		$.ajax({ 
			type: "post",
			url: "<?php  echo base_url() ?>tablas/tipoproducto/eliminarTipoproducto", cache: false, 
...
$(".editardato").click(function(event){
	...
	var url = "<?php  echo base_url() ?>tablas/tipoproducto/mantTipoproductoForm/"+idtablas;
	$( "#div_ModalMant" ).load(url);
});  

$("#btnnuevo").click(function(event){ 
	event.preventDefault();        
	var urlnuevo = "<?php  echo base_url() ?>tablas/tipoproducto/mantTipoproductoForm/";
