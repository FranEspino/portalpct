...
<div id="content">			
    <div class="row ">     
        <div class="col-xs-11">
            <h1>
                <i class="fa fa-users fa-fw "></i> Tipo de Usuario
            </h1>   
        </div>        
        <div class="col-xs-1 ">   
                        ...
                        <div class="table-responsive">
                            <table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
                            	<thead>			                
                                    <tr>
                                        <th>N.</th>
                                        <th><i class="fa fa-fw fa-user text-muted"></i>Tipo de Usuario</th>
								...
								<?php  if ($grid) {
                                     ...
                                     <tr id="<?php echo $fila->idusuario_tipo; ?>">
                                        <td><?php  echo $i; ?></td>
                                        <td><?php  echo $fila->nombre_usuario_tipo  ; ?></td>
                                        
                                        <td class="pull-right opciones hidden-print">
                                        <button  id="btn_editardato_<?php  echo $fila->idusuario_tipo ; ?>" name="editardato_<?php  echo $fila->idusuario_tipo ; ?>" value="<?php  echo $fila->idusuario_tipo ; ?>" class="editardato btn btn-primary btn-circle"  data-placement="left" rel="tooltip" data-original-title="Editar"><i class="glyphicon glyphicon-edit"></i></button>
                                        <a href="javascript:void(0);" onclick="elim_reg(<?php echo $fila->idusuario_tipo;?>);" class="btn btn-danger btn-circle" rel="tooltip" data-placement="bottom" title="Eliminar" ><i class="glyphicon glyphicon-trash"></i></a>
                                        </td>
                                    </tr> 
                                   <?php
								   $i++; 
 ...
 Swal.fire({
  ...
  if (result.value) {
	...
	if(idtabla.length<1){  
		...
		$.ajax({ 
			type: "post",
			url: "<?php  echo base_url() ?>tablas/usuario_tipo/eliminarUsuario_tipo", cache: false, 
...
$(".editardato").click(function(event){
	...
	var url = "<?php  echo base_url() ?>tablas/usuario_tipo/mantUsuario_tipoForm/"+idtablas;
	$( "#div_ModalMant" ).load(url);
});  

$("#btnnuevo").click(function(event){ 
	event.preventDefault();        
	var urlnuevo = "<?php  echo base_url() ?>tablas/usuario_tipo/mantUsuario_tipoForm/";
