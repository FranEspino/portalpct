...
<div id="content">			
    <div class="row ">     
        <div class="col-xs-12">
            <h1>
                <i class="fa fa-edit fa-fw "></i> Tipo de comprobantes
            </h1>   
        </div>     	 
    </div>     
    <!-- widget grid -->
                        ...
                        <div class="table-responsive">
                            <table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
                            	<thead>			                
                                    <tr>
                                        <th>N.</th>
                                        <th><i class="fa fa-fw fa-file-text text-muted"></i>Tipocomprobantees</th>
								...
								<?php  if ($grid) {
                                     ...
                                     <tr id="<?php echo $fila->id_tipocomprobante; ?>">
                                        <td><?php  echo $i; ?></td>
                                        <td><?php  echo $fila->tipocomprobante  ; ?></td>
                                        
                                        <td class="pull-right opciones hidden-print">
                                        	<?php if($fila->id_tipocomprobante>80){ ?>
                                        	<button  id="btn_editardato_<?php  echo $fila->id_tipocomprobante ; ?>" name="editardato_<?php  echo $fila->id_tipocomprobante ; ?>" value="<?php  echo $fila->id_tipocomprobante ; ?>" class="editardato btn btn-primary btn-circle"  data-placement="left" rel="tooltip" data-original-title="Editar"><i class="glyphicon glyphicon-edit"></i></button>
                                        	<?php } ?> 
                                        </td>
                                    </tr> 
                                   <?php
...
function elim_reg(id){	
	...
	if(idtabla.length<1){  
		...
		$.ajax({ 
			type: "post",
			url: "<?php  echo base_url() ?>tablas/tipocomprobante/eliminarTipocomprobante", cache: false, 
...
$(".editardato").click(function(event){
	...
	var url = "<?php  echo base_url() ?>tablas/tipocomprobante/mantTipocomprobanteForm/"+idtablas;
	$( "#div_ModalMant" ).load(url);
});  

$("#btnnuevo").click(function(event){ 
	event.preventDefault();        
	var urlnuevo = "<?php  echo base_url() ?>tablas/tipocomprobante/mantTipocomprobanteForm/";
