...
<div id="content">			
    <div class="row ">     
        <div class="col-md-8">
            <h1>
                <i class="fa fa-edit fa-fw "></i> Elementos
            </h1>   
        </div>        
        <div class="col-md-4 ">   
                        ...
                        <div class="table-responsive">
                            <table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
                            	<thead>			                
                                    <tr>
                                        <th>N.</th>
                                        <th><i class="fa fa-fw fa-user text-muted"></i>Descripción</th>
                                        <th>Accesorios</th>
                                        <th>Medida</th>
                                        <th>Cantidad</th>
                                        <th>Foto</th>
								...
								<?php  if ($grid) {
                                     ...
                                     <tr id="<?php echo $fila->tablas_id; ?>">
                                        <td><?php  echo $i; ?></td>
                                        <td><?php  echo $fila->elem_descripcion  ; ?></td>
                                        <td><?php  echo $fila->elem_desc_accesorios ; ?></td>
                                        <td><?php  echo $fila->elem_medida  ; ?></td>
                                        <td><?php  echo $fila->elem_cantidad  ; ?></td>
                                        <td><?php  echo $fila->elem_foto  ; ?></td>
                                        <td><?php  echo $fila->ttablas_id   ; ?></td>
                                        <td><?php  echo $fila->marca_id  ; ?></td>
                                        <td><?php  echo ($fila->est_id  =='1')? 'Activo':'Inactivo'; ?>
                                        </td>
                                        <td class="pull-right opciones hidden-print">
                                        <button  id="btn_editardato_<?php  echo $fila->tablas_id ; ?>" name="editardato_<?php  echo $fila->tablas_id ; ?>" value="<?php  echo $fila->tablas_id ; ?>" class="editardato btn btn-primary btn-circle"  data-placement="left" rel="tooltip" data-original-title="Editar"><i class="glyphicon glyphicon-edit"></i></button>
                                        <a href="javascript:void(0);" class="btn btn-danger btn-circle" data-trigger="focus" rel="popover" data-placement="bottom" data-original-title="<i class='fa fa-warning'></i> ¿Eliminar?" data-content='<button type="button" class="btn btn-default btn-sm" title="No eliminar"><span class="glyphicon glyphicon-menu-left"></span>Cancelar</button><legend class="no-padding no-margin" style="margin:2px !important"></legend><button onclick="elim_reg(<?php echo $fila->tablas_id;?>);" type="button" class="btn btn-danger btn-sm btn_si_eliminar" title="Si eliminar"  ><span class="glyphicon glyphicon-trash"></span> Eliminar</button>' data-html="true"><i class="glyphicon glyphicon-trash"></i></a>
                                        </td>
                                    </tr> 
                                   <?php
								   $i++; 
...
function elim_reg(id){	
	...
	if(idtabla.length<1){  
		...
		$.ajax({ 
			type: "post",
			url: "<?php  echo base_url() ?>tablas/eliminarElemento", cache: false, 
...
$(".editardato").click(function(event){
	...
	var url = "<?php  echo base_url() ?>tablas/mantElementoForm/<?php  echo $idtipotablas; ?>/"+idtablas;
	$( "#div_ModalMant" ).load(url);
});  

$("#btnnuevo").click(function(event){ 
	event.preventDefault();        
	var urlnuevo = "<?php  echo base_url() ?>tablas/mantElementoForm/<?php  echo $idtipotablas; ?>";
