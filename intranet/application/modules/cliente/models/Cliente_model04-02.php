...
class Cliente_model extends CI_Model {
 ...
 // === Razon social ===	 
	 ...
	 function gridRazonSocialSelect2($parametro){	 
		$sql = "SELECT id_razonsocial AS id, CASE WHEN id_razonsocial = '555' THEN CONCAT(ruc, ' - ',razonsocial) ELSE  CONCAT_WS(' ', ruc,CASE WHEN dni != '' THEN CONCAT (' (',dni,')') ELSE '' END ,'-',razonsocial) END AS text 
 FROM tb_razonsocial WHERE ruc LIKE '%".$parametro."%' OR dni LIKE '%".$parametro."%' OR razonsocial LIKE '%".$parametro."%'  ";
 ...
 WHERE `id_razonsocial` =  '".$id_razonsocial."' ORDER BY fecha_asignacion DESC LIMIT 1 ";
    ...
    function getMaxDNI(){	 
		$sql = "SELECT MAX(CAST(dni AS SIGNED)) maxdni FROM `tb_razonsocial` c WHERE LENGTH(CAST(dni AS SIGNED)) < 5";
		$query = $this->dbadmin->query($sql);
		if ($query->num_rows() > 0) {
            $data = $query->row();
        } else {
